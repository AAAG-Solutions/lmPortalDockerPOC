...
(function (window, $, undefined) {
    ...
    var JSGRID = "JSGrid",
        ...
        PAGE_SIZE_ITEM = "{pageSize}",
        PAGE_SIZE_ITEM_SAVE = "{pageButton}",
        PAGE_SIZE_ITEM_SAVE_TEXT = "<button id='pageSizeBtn' onclick='javascript:SetPageSize();return false;' style='width:35px;height:18px;text-align:-webkit-center;margin:0;padding:0;color:white;background-color:#005589;font-size:10px;'>Set</button>",
        PAGE_SIZE_ITEM_TEXT = "Items Per Page: <input type='text' id='pageSize' value='REPLACE_ME' style='width:30px;height:18px;text-align:-webkit-center'>",
    ...
    Grid.prototype = {
        ...
        _createPagerByFormat: function () {
            ...
            var vmList = [PAGE_SIZE_ITEM, PAGE_SIZE_ITEM_SAVE];
            ...
            return $.map(pagerParts, $.proxy(function (pagerPart) {
                ...
                if (pagerPart === PAGES_PLACEHOLDER) {
                    ...
                    result = this._addFormat(pageIndex);
                } else if (pagerPart === PAGE_COUNT_PLACEHOLDER) {
                    result = this._addFormat(pageCount);
                } else if (pagerPart === ITEM_COUNT_PLACEHOLDER) {
                    result = this._addFormat(itemCount);
                } else if (pagerPart === PAGES_ITEM) {
                    result = this._addFormat(pagerPart)
                } else if (pagerPart === TOTAL_ITEMS) {
                    result = this._addFormat("Total Items:")
                } else if (pagerPart === PAGE_SIZE_ITEM) {
                    result = this._addFormat(PAGE_SIZE_ITEM_TEXT.replace("REPLACE_ME", this.pageSize));
                } else if (pagerPart === PAGE_SIZE_ITEM_SAVE) {
                    result = this._addFormat(PAGE_SIZE_ITEM_SAVE_TEXT);
                } else {
                    result = this._addFormat(pagerPart)
                }
        ...
        _addFormat: function (text) {
            return $("<span>").addClass(this.pagerExtra).append(text);
        },
